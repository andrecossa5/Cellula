...
adata = sc.read('/Users/IEO6214/Desktop/Refractoring_test/data/default/integration.h5ad')
I = Int_evaluator(adata)
methods = I.methods
def metrics(adata, methods, k=15, n_components=30, covariate='seq_run', labels=None, resolution=0.2):
    all_functions = {'kBET': kbet,'entropy_bb': entropy_bb, 'graph_conn': graph_conn,
                     'kNN_retention_perc': kNN_retention_perc,'NMI': normalized_mutual_info_score, 'ARI': custom_ARI
                     }
    metrics_list={}
                ...
                if f in I.batch_metrics:    
                    if f != 'graph_conn':
                        ...
                        if m != 'scVI' and layer != 'raw' and m != 'original':
                            metrics_list.update({f+'|'+m+'|'+layer: [all_functions[f],[reps[m]],{'batch':adata.obs[covariate]}]})
