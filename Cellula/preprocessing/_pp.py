...
def compute_pca(adata, n_pcs=50, auto=True, biplot=False, GSEA=False, organism='human', colors=None):
    """
    Performs Principal Component Analysis (PCA) on some AnnData.
    Parameters
    ----------
    adata : AnnData
    ...
    biplot : bool, optional (default: False)
        Wheter to create biplots for the visual inspection of the top PCs
    GSEA : bool, optional (default: False)
        Wheter to create a GSEA reports on the gene loading of the top PCs
    ...
    Returns
    -------
    A tuple of np.array: (embeddings, loadings)
    """
    ...
    if GSEA:
        ...
        for i in range(1,6):
            ...
            fig.savefig(f'PC{i}_loadings.png')
